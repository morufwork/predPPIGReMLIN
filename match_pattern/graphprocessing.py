...
def read_graphs(filename,path=""):
	...
	for i in str_graphs:
		...
		i = re.sub(r'NP\.FLOAT64\(([^)]+)\)', r'\1', i)

		# Optional: strip any other numpy types (future-proof)
		i = re.sub(r'NP\.[A-Z0-9_]+\(([^)]+)\)', r'\1', i)

		graphs.append(nx.parse_gml(i))
...
def gspan_to_graph_queryq(filename,node_labels,edge_labels,multigraph=False):
	#filename = "NFPIso10/"+filename
...
def gspan_to_graph_queryt(filename,node_labels,edge_labels,multigraph=False):

	filename = "pattern/tquery0.7/"+filename
	with open(filename,"r") as file:
		patterns = [ [line for line in group.split("\n")] for group in file.read().split("\n\n")]
	graphs = []
