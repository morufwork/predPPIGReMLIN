...
def atom_provenance(atom, pdbid, is_ligand):
	residue = atom.get_parent()
	full_id = atom.get_full_id()
	hetero_flag, residue_number, insertion_code = residue.id
	coord_x, coord_y, coord_z = [float(value) for value in atom.coord]
	...
	return [
		...
		('residueInsertionCode', insertion_code.strip()),
		('heteroFlag', hetero_flag.strip()),
		('atomName', atom.get_name()),
		('atomSerial', int(atom.serial_number)),
		('coordX', coord_x),
		('coordY', coord_y),
		('coordZ', coord_z),
	]
	...
	for model_idx,model in sorted(contacts.items()):
		...
		###### Graph Generation
			...
			for contact in group:
				...
				s_data = atom_provenance(contact[0], pdbid, False)
				s_atom = full_id

				#Atom 2
				full_id = contact[1].get_full_id()
				d_data = atom_provenance(contact[1], pdbid, True)
				d_atom = full_id
				...
				if(g.has_edge(s_atom,d_atom)):
					...
					g.add_edge(s_atom,d_atom,type=typeCode[contact[2]],distance=float(contact[5]))
