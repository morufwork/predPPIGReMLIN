	...
	for p_graphs,graphs in zip(patterns,clusters):
		for min_sup, p_graphs in sorted(p_graphs.items(),key=lambda x: float(x[0])):
			for ip,p in enumerate(p_graphs):
				# p represent an instance in the entire pattern in the maximal patterns 
				pl = gp.line_graph(p)   # generate a line graph for the instance p
				str0 = ""
				for s in p.graph['ocur']: # retrieve the cluster ids at which an instance of maximal pattern p found
					#print(min_sup,ip,s)

					g = graphs[int(s)] # retrieve the graphs in that cluster ids (e.g. 0.5p as cl-ids{0,2,3,4,5,6,8,9,12})
					gl = g.graph["l_graph"] # generate a line graph for all graphs in each cluster id (e.g cl-0 as 16 graphs)
