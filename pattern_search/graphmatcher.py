...
def gen_gspan_clusters(graph_results,path=''):
	
	gsp = graph_results
	result = dict()
	for key,graph_list in sorted(gsp.items(),
										key=lambda x: x[0]):
	    #gSpanFName='%s.gsp'%key
		temp_gSpan_results = []
		for min_v, graphs in sorted(graph_list.items()):
			#print(min_v)
			#print(graph)
			for g in graphs:
				temp_gSpan_results.append(g)

		result[int(key)] = temp_gSpan_results

	return result
    

    		
def gen_iso_entries(clusters,node_labels,edge_labels,type_code,path=''):
...
def gen_iso_entries2(clusters,node_labels,edge_labels,type_code,path=''):
	...
	with cd(path+"/iso2"):
		...
		for key,graph_list in sorted(clusters.items(),
										key=lambda x: x[0]):
			#gSpanFName='%s.gsp'%key
			gp.multigraph_to_gspan2(graph_list,
					node_labels,edge_labels,type_code,gspan_fname=gSpanFName)

def read_iso_results(filename="iso2/entry.gsp",path=""):
    with open(Path(path)/"iso2/entry2.gsp","w") as file:
	    with open(Path(path)/filename, "r") as filename_list:
                i = 0
                for line in filename_list:
                    if line.startswith("t"):
                        #cprint(line)
                        #line = i.join(line.rsplit(line[-1:], 1))
                        #line = re.sub(r".$", str(i), line)
                        line1 = line.split("#")[0]
                        line2 = line1+"# " + str(i)+ "\n"
                        line = line2
                        i+=1
...
def gen_iso_creation(graph_list,node_labels,edge_labels,type_code,path=''):
...
def getMaximalGraphsN(query,dataGraph,file="map.json",path=""):	
	...
	#b = 0
	for min_sup, graphs in dataGraph.items():
		#b+=1
		g =	maximalN(query,graphs)
		temp[min_sup] = [ {"graph": j["graph"], "l_graph": j["l_graph"]} for j in g ]
		temp2.append([ {"graph": j["graph"], "l_graph": j["l_graph"]} for j in g ])
	...
	for g in range(len(graphs)):
		...
		for i in range(len(graphs[g])):
			if marked[g][i]:
				continue
			#print(query["l_graph"])
			m = nx.isomorphism.GraphMatcher(graphs[g][i]["l_graph"],query["l_graph"],edge_match=em,node_match=nm)
...
def getMaximalGraphs(query,clusters,file="maximal.json",path=""):	
