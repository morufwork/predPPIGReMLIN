...
def read_gSpan_results_queryq(node_labels,edge_labels,filename,path=""):
	...
	with cd(path):		
		for cluster,files in result_files['results'].items():
			...
			for filename in files:
				key = filename.split('_')[1].split('f')[0][:-1]
				temp_results[key] = gp.gspan_to_graph_queryq(filename,node_labels,edge_labels)
			
			gSpan_results[int(cluster)] = temp_results
	return gSpan_results,supports
		
    
def read_gSpan_results_queryt(node_labels,edge_labels,filename,path=""):
	...
	with cd(path):		
		for cluster,files in result_files['results'].items():
			...
			for filename in files:
				key = filename.split('_')[1].split('f')[0][:-1]
				temp_results[key] = gp.gspan_to_graph_queryt(filename,node_labels,edge_labels)
...
def mapGraphsTest(clusters,patterns,supports,path=""):
...
def jsonParse(clusters,atom_types,supports,type_code,typenames,path=""):
	...
	with (path/'group-info.csv').open(mode="w") as gFile:
		...
		for g in graphs:
			...
			g = g.graph
			gFile.write('%d,%d,"%s","%s","%s"\n'%(k,nNode,g['pdbid'],g['source'],g['target']))
		...
		with cd(path/dir_path):
			...
			for k,v in sorted(clusters.items()):
				for graphs in v:
					...
					#g = {'data':{'nodes': [], 'links': []},'meta':tuple([k+1]+[graphs.graph[x] for x in keys])}
					keys = ['pdbid','source']
					g = {'data':{'nodes': [], 'links': []},'meta':tuple([len(graphs)]+[graphs.graph[x] for x in keys]+[k])}
					### Nodes
					for node in graphs.nodes():
						...
						resAtom = node['residueName']+" : "+node['atomName']
						try:
					...
					for edge in graphs.edges(data=True):
						...
						try:
							temp_edge['interactionType'] = "/".join(typenames[t] for t in interactionType)
							temp_edge['distance'] = str(edge[2]['distance'])
							temp_edge['interactionTypeInt'] = str(reduce(lambda x,y: x*y,[typeInt[t] for t in interactionType]))
	...
	def Remove(duplicate):
		...
		return final_list
	efile = Remove(error_graphs)

	with cd(path):
		# open file in write mode
		with open('errorFile2.txt', 'w') as fp:
			for item in efile:
	...
	vert_number_array = [["group","patternSize","occurrences"]]
	test = []
	for key, graphs in clusters.items():
		patterns_info = {}
		for g in graphs:
			patterns_info[len(g)] = len(g)
		test.append(patterns_info)
	

	occurInfo = counter([list(i.values())[0] for i in test])
	for k,v in sorted(occurInfo.items()):
		vert_number_array.append([k,k,v])
...
def maximalCountNew(interDict,node_labels,edge_labels,typenames,type_code,path=""):
	data_dir_name = 'data'
	path = Path(path) / data_dir_name
	...
	p_edge_labels = set()
	p_node_labels = set()
	#for g_patterns in subgraph_dict:
	for sup, s_group in interDict.items():
		for p in s_group:
			#print(p)
	...
	count_matrix = [['group', 'vertices'] + count_matrix] 
	
	#print(count_matrix)
	#for cl_idx in range(len(interDict)):
		#for min_sup,v in interDict[cl_idx].items():
	for min_sup,v in sorted(interDict.items()):
		line = [min_sup,min_sup] + [0]*columns
